...
c
 ...
 10   if (2**ncube.lt.nprocs) then
     ...
     $     iunknownflag,ihandflag,isyncflag,icopyflag)
      ...
      if (ipartner.eq.-1) ipartner = mpi_any_source
      if (itype.eq.-1) itype = mpi_any_tag
      call mpi_recv(buf,isize,mpi_byte,ipartner,itype,
     $     mpi_comm_world,status,ierror)
      if (ierror.ne.0) write (6,*) 'MPI_RECV error:',ierror
      ipartner = status(mpi_source)
      itype = status(mpi_tag)
      call mpi_get_count(status,mpi_byte,iactual,ierror)
      nread = iactual
      ...
      call mpi_send(buf,isize,mpi_byte,ipartner,itype,
